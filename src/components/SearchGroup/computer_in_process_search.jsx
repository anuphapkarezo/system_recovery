...
function Computer_In_Process_Search_Group({ onSearch }) {
    ...
    //Set Dropdown List
    const [selecteddivision, setSelecteddivision] = useState({ division: "Division" });
    const [selectedDepartment, setSelectedDepartment] = useState({ dep_unit: "Department" });
    const [selectedCostCenter, setSelectedCostCenter] = useState({ cost_center_name: "Cost Center" });
    ...
    const fetchdivision = async () => {
        try {
          const response = await axios.get("http://10.17.66.242:3001/api/smart_recovery/filter-division-list");
    ...
    const fetchDepartment = async (division) => {
        ...
        try {
          const response = await axios.get(`http://10.17.66.242:3001/api/smart_recovery/filter-department-list?division=${division}`);
    ...
    const fetchCostCenter = async () => {
        ...
        try {
          const response = await axios.get(`http://10.17.66.242:3001/api/smart_recovery/filter-costcenter-list?division=${selecteddivision.division}&department=${selectedDepartment.dep_unit}`);
        ...
        <React.Fragment>
            <Box maxWidth="xl" sx={{ width: "100%" , height: 50}}>
                {/* <div className="columns is-desktop">
                    <div style={{width: 220 , backgroundColor: '#80B3FF' , height: 70 , marginLeft: 25 , borderRadius: 10 ,marginTop: 20}}>
                        <p style={{color: 'white' , fontSize: 22 , fontWeight: 'bold' , paddingTop: 3}}>TOTAL PC</p>
                        <p style={{color: 'white' , fontSize: 20 , fontWeight: 'bold'}}>240</p>
                    </div>

                    <div style={{width: 220 , backgroundColor: '#6A9C89' , height: 70 , marginLeft: 20 ,marginTop: 20,borderRadius: 10}}>
                        <p style={{color: 'white' , fontSize: 22 , fontWeight: 'bold' , paddingTop: 3}}>PC CONNECT</p>
                        <p style={{color: 'white' , fontSize: 20 , fontWeight: 'bold'}}>150</p>
                    </div>

                    <div style={{width: 220 , backgroundColor: '#FFCF96' , height: 70 , marginLeft: 20 ,marginTop: 20,borderRadius: 10}}>
                        <p style={{color: 'white' , fontSize: 22 , fontWeight: 'bold' , paddingTop: 3}}>WAIT CONNECT</p>
                        <p style={{color: 'white' , fontSize: 20 , fontWeight: 'bold'}}>190</p>
                    </div>
                </div> */}
                <Grid container spacing={0} style={{width: 1350 }}> 
                    ...
                    <Grid  item xs={2} md={2} >
                        <div style={{ display: 'grid', placeItems: 'center' }}>
                            <Autocomplete
                                ...
                                sx={{ width: 250 , height: '60px' , marginTop: '8px' , marginLeft: '55px'}}
                    ...
                    <Grid  item xs={2} md={2} >
                        <div style={{ display: 'grid', placeItems: 'center' }}>
                            <Autocomplete
                                ...
                                sx={{ width: 250 , height: '60px' , marginTop: '8px' , marginLeft: '93px'}}
                                renderInput={(params) => <TextField {...params} label="Cost Center" />}
                                isOptionEqualToValue={(option, value) =>
                                    option && value && option.cost_center_name === value.cost_center_name
                                }
                            />
                        </div>
                    </Grid>
                    <Grid  item xs={2} md={2} >
                        <Button 
                            variant="contained" 
                            // size="small"
                            style={{width: 150, height: '40px' , marginTop: '15px', marginLeft: '130px' , borderRadius: 30}}
                    ...
                    <Grid  item xs={2} md={2} >
                        <Button 
                            variant="contained" 
                            // size="small"
                            style={{width: 150, height: '40px' , marginTop: '15px', marginLeft: '60px' , borderRadius: 30}}
                            >Request SE
                        </Button>
                    </Grid>
                </Grid>
